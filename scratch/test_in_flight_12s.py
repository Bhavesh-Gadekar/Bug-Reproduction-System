...
payload = {
    "title": "dateutil issue 981 in-flight 12s test",
...
# 3. Poll DB until system_init appears
print("Waiting for run to enter active execution...")
active = False
...
for _ in range(60):
    with conn.cursor() as cur:
        ...
        if "system_init" in nodes:
            print(f"Active run detected in DB: {nodes}")
            active = True
            break
    time.sleep(0.5)

assert active, "Run did not start within 30s!"

# Give 1.5 seconds so it enters repo_analysis (actively running in worker)
time.sleep(1.5)

# 4. Kill Redis connection for 12 seconds (guaranteed to exceed redis retry limit)
print(f"Killing Redis for 12s while run {run_id} is in-flight...")
...
# 5. Wait 8 seconds for worker container supervisor to record crash, exit, and Docker to restart
time.sleep(8)
